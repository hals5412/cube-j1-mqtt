...
さらに正常時の計測ログ(`Measurements:`)は1時間に1回へ間引き(`MEASUREMENT_LOG_INTERVAL`)、エラー・状態変化・起動/復旧後の初回計測は常に記録する。これにより同じ2MBで数ヶ月分の履歴を遡れる。ログはADB無効でも書き続けられ、障害後にADBを有効化すれば過去の記録を事後解析できる。
