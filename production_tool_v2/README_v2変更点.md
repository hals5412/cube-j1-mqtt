...
旧版は `/data/local/mqtt_bridge.log` への無制限追記で、長期稼働や再接続ループ時に `/data` パーティションを圧迫する恐れがあった。v2では1MB到達時に `.1` へ世代交代し(旧 `.1` は削除)、ログの合計サイズを最大約2MBに制限する。旧版から引き継いだ肥大化済みログも、v2起動後の最初のログ書き込みでローテーションされる。

さらに正常時の計測ログ(`Measurements:`)は1時間に1回へ間引き(`MEASUREMENT_LOG_INTERVAL`)、エラー・状態変化・起動/復旧後の初回計測は常に記録する。ログはADB無効でも書き続けられ、障害後にADBを有効化すれば過去の記録を事後解析できる。

ログは `config.json` で調整できる(どちらも省略可):

| キー | 既定値 | 説明 |
|---|---|---|
| `log_max_bytes` | `10485760`(10MB) | この サイズで `.1` へ世代交代。合計は最大約2倍 |
| `log_enabled` | `true` | `false` でファイル書き込み自体を無効化(事後解析用の記録は残らなくなる点に注意) |

## 長期運用対策

- **discovery定期再publish**: discovery設定をMQTT再接続後および24時間ごと(`DISCOVERY_REPUBLISH_INTERVAL`)に再publishする。ブローカーのretainデータが消えた場合(MQTTブローカー再構築など)でも、本体を再起動せずHA上のエンティティが自動復元される。同一payloadの再publishはHAの表示・値・履歴に影響しない。
- **送信キュー上限**: MQTT再送キューに上限1000件を設定。超過時は最も古い値から捨てる(最新値で代替できるため連携データへの実害なし)。
- **ADB設定の対称化**: インストーラがADB TCPを `install_config.sh` の値どおりの状態へ必ず揃える。旧版は有効化処理しか持たず、一度 `PERSIST_ADB=1` で永続化すると `0` に戻して再導入してもオンのままだった。v2では `ENABLE_ADB=0` で永続設定ごと解除、`PERSIST_ADB=0` なら永続分のみ解除される。
