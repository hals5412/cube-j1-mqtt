...
さらに、起動時にメーターのプロパティマップ（EPC: 9F）を取得し、**そのメーターが対応する場合のみ**以下を追加でポーリングします（適応ポーリング）。未対応の項目は Home Assistant の MQTT 自動検出からも除外され、不要な `unknown` センサーが残らないようにしています。
...
7. セットアップ完了を確認したら、USB メモリを抜いて構いません。必要なファイルは Cube J1 本体へ保存されるため、以後は USB メモリを挿さなくても、電源投入時に MQTT ブリッジが自動起動します。
...
| `poll_interval` | スマートメーターへデータを取得しに行くポーリング間隔（秒）。安全のため30秒未満は30秒に補正される |
...
- **自己復旧**: ERXUDP 無応答が 3 回連続すると Wi-SUN を再 join します。再 join はバックオフ付き（30〜300 秒）で成功までリトライし、失敗が 5 回続くたびにシリアルポートを開き直します。PANA 認証失敗（EVENT 24）も再 join の契機になります。EVENT 29（セッション失効）後に応答が途絶えた場合は、通常の3回を待たず再 join します。
- **ポーリング周期**: 前回のポーリング開始時刻を基準に次回開始を調整するため、応答待ち時間がそのまま設定間隔へ上乗せされません。
- **Wi-SUNモジュール保護**: `ROPT` で既存設定を確認し、必要な場合だけ `WOPT 1` を実行して不要なフラッシュ書き込みを避けます。

### 開発者向けテスト

Linux または WSL 上で、Python 標準の `unittest` を使って実行できます。追加パッケージは不要です。
...
別フォーク [enkunkun/cube-j1-mqtt](https://github.com/enkunkun/cube-j1-mqtt) の検証結果を参考にしつつ、環境依存の強い積極的な再接続設定は取り込まず、影響範囲の小さい制御だけを選択して実装しています。
...
- ポーリング中の EVENT 24（PANA 認証失敗）で即再 join
- EVENT 29（セッション失効）はモジュールの自動再認証を待ち、同じポーリングで応答も失敗した場合だけ即再 join
- `ROPT` で現在値を確認し、設定済みの場合は `WOPT 1` を省略してモジュールのフラッシュ書き込みを抑制
- ポーリング開始時刻基準の周期制御と30秒の安全下限を追加
...
Cube J1 標準状態では、スマートフォンアプリでの初期設定用と思われる Wi-Fi Direct / P2P AP（例: `CubeJ-xxxxxx`）が見える場合がある。Home Assistant 連携では通常の自宅 Wi-Fi と Wi-SUN B ルートだけを使うため、この AP は不要です。
...
- 独自の堅牢化を追加:
  ...
  - ECHONET Lite の TID を照合し、古い応答や別リクエストの応答を計測値として取り込まない
  - 追加 EPC だけ応答が返った場合は正常取得扱いにせず、瞬時電力・積算電力量などの基本計測値が取れない状態を検知する
  - 追加センサーは対応 EPC が確認できた場合だけ Home Assistant に登録し、未対応項目の retained discovery は自動で削除する
