...
3. `production_tool/wpa_supplicant.conf.example` を `production_tool/wpa_supplicant.conf` にコピーし、Wi-Fi の SSID とパスワードを設定する（実ファイルは認証情報を含むため Git 管理対象外）。
...
- 元の `.rc` は導入時に `/data/local/cubej1-backup/` へ自動バックアップされる
