...
既定は `ENABLE_ADB=1, PERSIST_ADB=0` とし、今回の起動中だけログ取得・遠隔復旧手段を有効にします。電源再投入後もADBを残す必要がある導入時だけ `PERSIST_ADB=1` に変更してください。永続有効はLAN内にroot権限のADBアクセスを開くため、信頼できるLAN内でのみ使用してください。
