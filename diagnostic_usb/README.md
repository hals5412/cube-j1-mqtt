...
診断USBでWi-Fiを確認する場合だけ、`wpa_supplicant.conf.example` を `wpa_supplicant.conf` にコピーしてから、SSIDとパスワードをローカルで編集してください。本番導入用のWi-Fi設定は `production_tool/wpa_supplicant.conf` を直接編集します。
